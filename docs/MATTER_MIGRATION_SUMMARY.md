...
Successfully migrated the 23 hardcoded matters from the Matters view to Firestore database, following the Solo Team Architecture schema. The implementation includes full CRUD operations, reactive data management, and a development seeding utility.
...
- ✅ Retrieve single matter or all team matters
...
**Pattern:** Static class methods following `TeamService` conventions

**Storage Path:** `/teams/{teamId}/matters/{matterId}` (matterId is auto-generated by Firestore)
...
- ✅ Automatic teamId integration via auth store
...
**Features:**
- ✅ Visual confirmation of current user and team
...
5. **Verify success:**
   ...
   - Or open Firebase Console → Firestore → teams → {your-userId} → matters
...
Path: `/teams/{teamId}/matters/{matterId}`
...
```javascript
{
  matterNumber: '2024-001',           // Human-readable ID (editable, unique per team)
...
- ✅ **Duplicate prevention:** Application-layer validation ensures unique matter numbers per team
...
### Solo Team Architecture Compliance
- ✅ All data scoped to `teamId` (for solo users: `teamId === userId`)
- ✅ Security rules automatically enforce team-based access
- ✅ Seamless upgrade path to multi-user teams
...
1. **Check Firebase Console:**
   - Open Firestore → teams → {your-userId} → matters
...
3. **Verify authentication:**
   ...
   - Check `authStore.teamId` is set
   - Check `authStore.user.uid` exists

### Permission denied errors

1. **Check Firestore security rules:**
   - Rules should allow team-based access
   - Path: `/teams/{teamId}/matters/{matterId}`
   - Rule: `request.auth.token.teamId == teamId`

2. **Verify custom claims:**
   - User should have `teamId` in custom claims
   - For solo users: `teamId === userId`

### Seeding fails

1. **Check you're logged in:**
   - Auth store should have user and teamId
...
- **Schema Documentation:** `docs/architecture/SoloTeamMatters.md`
- **Security Rules:** `docs/architecture/security-rules.md`
- **Authentication Guide:** `docs/authentication.md`
- **Team Workflows:** `docs/architecture/team-workflows.md`
