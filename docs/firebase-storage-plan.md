...
- **Team general** (reserved): `matter-general` - For team resources, templates, and non-client documents
- **Regular matters**: Auto-generated Firestore document IDs (e.g., `abc123def456ghi789`)

**Note**: Client names are stored directly in matter documents as string arrays, not as ID references. See [SoloTeamMatters.md](./architecture/SoloTeamMatters.md) for complete matter schema.
...
Each team has one reserved matter ID:

- **`matter-general`** - Team resources, templates, and non-client documents

All other matter IDs are auto-generated by Firestore when creating new matter documents.

### Multi-Client Matters

Matters can have multiple clients - client names are stored as a string array directly in the matter document. For complete matter data structure including the `clients[]` field, see [SoloTeamMatters.md](./architecture/SoloTeamMatters.md).
...
- **Team files** → Reserved `matter-general` ID
...
const matterRef = await db.collection('teams').doc(teamId).collection('matters').add({
...
uploadFile(file, teamId, matterId);
```
...
- [ ] Upload logging
- [ ] Create reserved `matter-general` system matter
...
await uploadFile(consultationNotes, teamId, matterId);
await uploadFile(retainerAgreement, teamId, matterId);
```
...
await uploadFile(estatePlanningDocs, teamId, matterId);
await uploadFile(jointAssets, teamId, matterId);
```
...
await storageService.uploadFile(
  file,
  teamId,
  matterId,  // Firestore auto-generated ID or 'matter-general' for firm files
  {
...
// Upload to team resources (reserved ID)
await storageService.uploadFile(
  file,
  teamId,
  'matter-general', // Reserved ID for team-wide files
  {
