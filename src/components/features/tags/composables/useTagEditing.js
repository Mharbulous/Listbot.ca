...
export function useTagEditing(tag, isOpenCategory, categoryOptions, emit) {
  ...
  const isValidPrefix = (prefix) => {
    ...
    return categoryOptions.some((option) => option.tagName.toLowerCase().startsWith(lowerPrefix));
  };
  ...
  const handleTagBlur = (event) => {
    ...
    setTimeout(() => {
      ...
      const focusStillInTag =
        relatedTarget && smartTagContainer && smartTagContainer.contains(relatedTarget);
  ...
  const handleTypeToFilter = (event) => {
    ...
    if (capitalizedText.length > 0) {
      ...
      console.log(
        `Set hasStartedTyping=true for text: "${capitalizedText}" (length: ${capitalizedText.length})`
      );
  ...
  const getTopFilteredOption = () => {
    ...
    return (
      categoryOptions.find((option) => option.tagName.toLowerCase().startsWith(filter)) ||
      categoryOptions[0]
    );
  };
      ...
      if (newValue && newValue !== tag.tagName) {
        ...
        if (hasTrailingSpace) {
          ...
          const exactMatch = matchingOptions.find(
            (option) => option.tagName.toLowerCase() === trimmedValue.toLowerCase()
          );
          if (exactMatch) {
  ...
  const selectFromDropdown = (newValue) => {
    ...
    console.log(
      `Dropdown selection completed in ${Math.round((endTime - startTime) * 100) / 100}ms`
    );
  };
