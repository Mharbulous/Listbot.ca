  ...
  <div class="document-table" style="min-width: 0">
    ...
    >
      ...
      <div class="table-mockup-header" :style="{ minWidth: totalFooterWidth + 'px' }">
        ...
        <ControlColumn
          ref="headerControlColumn"
          variant="header"
          :columnSelectorLabel="columnSelectorLabel"
          @column-selector-click="showColumnSelector = !showColumnSelector"
        />
      ...
      <div class="table-mockup-body" :style="{ minWidth: totalFooterWidth + 'px' }">
        ...
        >
          ...
          >
            <!-- Action Buttons Cell -->
            <ControlColumn
              :ref="(el) => setRowControlColumnRef(el, sortedData[virtualItem.index].id)"
              variant="row"
              :width="COLUMN_SELECTOR_WIDTH"
              :row="sortedData[virtualItem.index]"
              :isPeekTooltipVisible="tooltipTiming.isVisible.value"
              @process-ai="handleProcessWithAI"
              @peek-click="handlePeekClick"
              @peek-dblclick="handlePeekDoubleClick"
              @peek-mouseenter="handlePeekMouseEnter"
              @peek-mouseleave="handlePeekMouseLeave"
            />
...
import ControlColumn from '@/components/base/ControlColumn.vue';
...
const headerControlColumn = ref(null);
const rowControlColumnRefs = ref(new Map());
...
const updateTooltipPosition = (fileHash) => {
  const controlColumn = rowControlColumnRefs.value.get(fileHash);
  if (!controlColumn) return;

  const button = controlColumn.peekButton;
  if (!button) return;
...
// Set ref for row control column component
const setRowControlColumnRef = (el, fileHash) => {
  if (el) {
    rowControlColumnRefs.value.set(fileHash, el);
  } else {
    rowControlColumnRefs.value.delete(fileHash);
  }
};
...
// Computed ref for column selector button (accessed through header control column)
const columnSelectorBtn = computed(() => {
  return headerControlColumn.value?.columnSelectorBtn || null;
});
