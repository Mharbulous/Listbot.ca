...
const categorySource = ref(route.query.source || 'firm');
const categoryManager = useCategoryManager();

// Form helpers (regex validation, system category detection)
const { autoGeneratedExamples, isRegexValid, isSystemCategoryComputed } = useCategoryFormHelpers(
  ref({ type: '' }), // Will be replaced by editedCategory
  route
);

// Form state and validation
const { editedCategory, categoryErrors, isFormValid, hasChanges, capitalizeFirstLetters, validateCategory } =
  useCategoryEditForm(category, isRegexValid);

// Update form helpers to use actual editedCategory
const formHelpers = useCategoryFormHelpers(editedCategory, route);
const autoGeneratedExamplesActual = formHelpers.autoGeneratedExamples;
const isSystemCategoryComputedActual = formHelpers.isSystemCategoryComputed;
...
const {
  ...
  confirmTypeChange,
  cancelTypeChange,
} = useCategoryTypeConversion(editedCategory, originalType);

// CRUD actions
const { loading, saving, snackbar, loadCategory, saveCategory, deleteCategory } =
  useCategoryEditActions({
    ...
    router,
    validateCategory,
    autoGeneratedExamples: autoGeneratedExamplesActual,
  });

// Use the actual computed values
const autoGeneratedExamples = autoGeneratedExamplesActual;
const isSystemCategoryComputed = isSystemCategoryComputedActual;
