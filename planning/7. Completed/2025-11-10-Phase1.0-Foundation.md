...
**Deliverable:** Two upload buttons + standard scrolling table + status system + footer with UPLOAD button
...
- ✅ **Upload buttons moved to AppHeader** - "+ Add to Queue" dropdown menu in header (not separate component when queue empty)
- ✅ **TWO upload buttons only** - "Upload Files" and "Upload Folder + Subfolders" (root-only option deliberately removed per research/2025-11-11-RemoveExcludeSubfoldersOption.md)
- ✅ **Empty state integrated into UploadTable** - Shows drag-and-drop zone when queue is empty
- ✅ **Select All checkbox column** added as FIRST column (60px) - not in original plan
- ✅ **No Actions column** - original plan had Actions column, but not implemented
- ✅ **Eyeball icon (👁️) for file preview** - Opens file in browser tab on hover (implemented early, was planned for Phase 7)
- ✅ **Footer has TWO buttons**: "Clear X skipped files" (white) + "Upload X files" (green) instead of just UPLOAD
- ✅ **Skip/Undo system** - Files marked as 'skip' status instead of removing from queue
...
## Part A: Upload Initiation (Two-Button System)
...
- **Folders:** Automatically extracts ALL files from the folder and its subfolders (recursive - always includes all nested files)
- **Mixed:** Processes both files and folders in a single drop

**Important:** Drag-and-drop of folders ALWAYS includes all subfolders recursively. This matches the "Upload Folder + Subfolders" button behavior and aligns with legal tech industry standards (see research/2025-11-11-RemoveExcludeSubfoldersOption.md).

### A.2 Two Upload Buttons

**Rationale for Two Buttons Only:**
Based on extensive legal tech industry research (documented in research/2025-11-11-RemoveExcludeSubfoldersOption.md), the "Upload Folder" (root-only) option was deliberately removed. All major legal document management platforms (Clio, NetDocuments, iManage, Relativity, Everlaw, Logikcull, DISCO) universally implement recursive-only folder upload. No documented user demand exists for excluding subfolders, and UX research strongly supports simplification when 95%+ of users would make the same choice.
...
│                                                                 │
│  ┌──────────────────────────┐  ┌──────────────────────────────┐│
│  │ Upload Files             │  │ Upload Folder + Subfolders   ││
│  │                          │  │                              ││
│  └──────────────────────────┘  └──────────────────────────────┘│
│                                                                 │
└────────────────────────────────────────────────────────────────┘
...
#### Button 2: Upload Folder + Subfolders
**Icon:** 📂
...
**Icon:** 📦
**Behavior:** Recursive folder traversal (same as Button 2 - Upload Folder + Subfolders)
...
```
┌────────┬──────┬───────────────────────────┬──────────┬──────────────┬──────────┬─────────┐
│ Select │ Type │ File Name                 │ Size     │ Folder Path  │ Modified │ Status  │
│ (60px) │(40px)│ (flex: 1, max 500px)      │ (100px)  │ (130px min,  │ (140px)  │ (100px) │
│        │      │                           │          │  max 500px)  │          │         │
├────────┼──────┼───────────────────────────┼──────────┼──────────────┼──────────┼─────────┤
│ [✓]    │ 📄   │ invoice.pdf 👁️            │ 2.4 MB   │ /2024/Tax    │ 2h ago   │ 🔵 Ready│
│ [✓]    │ 📊   │ report-very-long-name...  │ 890 KB   │ /Reports     │ 1d ago   │ 🟡 Up...│
│ [ ]    │ 📄   │ form.pdf                  │ 1.2 MB   │ /Forms       │ 3d ago   │ ⚪ Skip  │
├────────┴──────┴───────────────────────────┴──────────┴──────────────┴──────────┴─────────┤
│ Footer:                                                                                  │
...
**Note:** Eyeball icon (👁️) appears on hover over file name for browser preview.
...
- **File Name column**: Flexible width (flex: 1) - expands to fill remaining space, with eyeball icon (👁️) on hover for browser preview
- **Eyeball icon preview**: Implemented early (was planned for Phase 7) - opens file in new browser tab using `URL.createObjectURL()` instead of modal preview
...
| ⚪ | White/Gray | `skip` | Skip | User manually skipped (can undo) |
| 🟣 | Purple | `skipped` | Duplicate | Detected as duplicate |
| 🔴 | Red | `error` | Failed | Upload failed with error |
| 🟠 | Orange | `n/a` | N/A | Unsupported file type (e.g., .lnk, .tmp) |
| ⚪ | White | `uploadMetadataOnly` | Metadata Only | Uploaded metadata only |
| ⚫ | Gray | `unknown` | Unknown | Unknown status (fallback) |

**Notes:**
- **`skip` status** (White): User manually skipped with checkbox (can undo). Counted in "Skipped" footer stat.
- **`n/a` status** (Orange): Unsupported file types that cannot be uploaded (e.g., .lnk, .tmp). Shows ⛔ emoji instead of checkbox. Cannot be selected. Counted in "Skipped" footer stat.
- **`skipped` status** (Purple): Automatically detected as duplicate file. Different from manual skip.
- **Orange is now N/A**: Changed from `uploadMetadataOnly` (now uses White) to `n/a` (unsupported files).

```css
.status-ready { background-color: #2196F3; }
.status-uploading { background-color: #FFC107; }
.status-completed { background-color: #4CAF50; }
.status-skip { background-color: #FFFFFF; border: 1px solid #9e9e9e; }
.status-skipped { background-color: #9C27B0; }
.status-error { background-color: #F44336; }
.status-n\/a { background-color: #FF9800; }
...
src/features/upload/
├── components/
│   ├── UploadButtons.vue            # Upload buttons with drag-drop (exists but NOT currently used)
│   ├── UploadTable.vue              # Main table orchestrator (business logic, empty state, footer stats computation)
│   ├── UploadTableVirtualizer.vue   # ⭐ Scroll container + header + rows + footer (isolation layer for Phase 1.5)
│   ├── UploadTableHeader.vue        # Sticky header with Select All checkbox (INSIDE virtualizer)
│   ├── UploadTableRow.vue           # Individual row with checkbox
│   ├── UploadTableFooter.vue        # Footer with Clear + Upload buttons (INSIDE virtualizer)
│   ├── StatusCell.vue               # Status dot + text (if exists)
│   └── QueueProgressIndicator.vue   # Non-blocking progress bar
└── composables/
    └── useUploadTable.js            # Table state management + skip/undo logic
```
...
- ⭐ **ARCHITECTURE:** `UploadTableVirtualizer.vue` contains the complete scrolling region
  - **Phase 1.0:** Uses standard v-for rendering (~200 lines)
  - **Phase 1.5:** Will integrate TanStack Virtual (~400-500 lines)
  - **Purpose:** Eliminates fragile scrollbar width calculations by placing header & footer inside scroll container
  - **Contains:** Header (sticky top) + Rows (standard/virtual) + Footer (sticky bottom)
  - **Benefits:** Natural alignment, prepares for virtualization, follows standard patterns
- No `useVirtualScroll.js` in Phase 1.0 - that will be added in Phase 1.5
...
- [x] Implement two file input types (files, folder-recursive)
- [x] ~~Add root-only filtering logic for Button 2~~ (removed per research/2025-11-11-RemoveExcludeSubfoldersOption.md)
...
- [x] Define 9 status types (added 'skip' for user-controlled skipping, 'n/a' for unsupported files)
...
- [x] **CHANGE:** Added 'n/a' status for unsupported file types (.lnk, .tmp) - shows ⛔ emoji instead of checkbox
- [ ] Test all 9 status states
...
- [x] Two upload buttons work correctly (via AppHeader "+ Add to Queue" dropdown)
- [x] Files appear in table immediately after selection
- [x] Table displays with all 7 columns (Select | File Type Icon | File Name | Size | Folder Path | Modified | Status)
...
| Two-button system not intuitive | Low | Low | Clear button labels, hover descriptions, matches industry standards |
...
**Phase Status:** 🟢 Core Implementation Complete (Testing and polish pending)
**Last Updated:** 2025-11-11
**Implementation Notes:**
- ✅ All upload buttons working (via AppHeader "+ Add to Queue" dropdown)
- ✅ Table structure complete with flex-based layout
- ✅ Header with Select All checkbox implemented
- ✅ Footer with Clear + Upload buttons implemented
- ✅ Status system with 9 status types (including 'skip' for manual skipping, 'n/a' for unsupported files)
- ✅ Responsive column widths (Select: 60px, File Name: flex, Size: 100px, Folder Path: 130px, Status: 100px)
- ✅ N/A file type handling (⛔ emoji, orange dot, excluded from selection, counted in skipped)
- ✅ Skip/Undo system for file management
- ✅ Select All / Deselect All functionality
- ✅ Empty state with drag-and-drop integrated into UploadTable
- ✅ Queue progress indicator for large batches
- ✅ **NEW:** UploadTableVirtualizer.vue created to isolate virtualization complexity
- ⏳ Upload logic implementation pending (TODO button just logs)
...
7. ⭐ **NEW:** UploadTableVirtualizer.vue extracts scroll/rendering concerns (Phase 1.5 prep)
