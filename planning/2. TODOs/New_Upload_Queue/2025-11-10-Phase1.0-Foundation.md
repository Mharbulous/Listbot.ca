...
│   ├── UploadTable.vue              # Main table orchestrator (business logic, empty state)
│   ├── UploadTableVirtualizer.vue   # ⭐ NEW: Scroll container + row rendering (isolation layer for Phase 1.5)
│   ├── UploadTableHeader.vue        # Sticky header with Select All checkbox
│   ├── UploadTableRow.vue           # Individual row with checkbox
│   ├── UploadTableFooter.vue        # Footer with Clear + Upload buttons
...
- ⭐ **NEW:** `UploadTableVirtualizer.vue` isolates scroll/rendering logic from business logic
  - **Phase 1.0:** Uses standard v-for rendering (~100 lines)
  - **Phase 1.5:** Will integrate TanStack Virtual (~300-400 lines)
  - **Purpose:** Defend against scope creep by clearly separating virtualization concerns
